...
import { BarChart3, Globe, Smartphone, Briefcase, Code, Database, Cloud, User, PlayCircle } from 'lucide-react'
...
const FeatureIcon = ({ icon: Icon, x, y, color }: { icon: React.ElementType; x: number; y: number; color: string, rotate?: number }) => {
  ...
  return (
    <motion.g
      ...
      onHoverStart={() => controls.start({ y: -10, scale: 1.2 })}
...
// Enhanced Animated Diagram Component
const EnhancedAnimatedDiagram = () => {
  ...
  return (
    <svg viewBox="0 0 400 400" className="w-full h-auto max-w-md mx-auto" onMouseMove={handleMouseMove} onMouseLeave={() => setCursorPosition({ x: 100, y: 185 })}>
      <defs>
      ...
      <motion.circle cx="200" cy="200" r="50" fill="url(#bgGrad)" stroke="rgba(0, 0, 0, 0.2)" strokeWidth="2" filter="url(#glow)" initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ duration: 2, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }} />

      {/* Rotating Circle Layers */}
      <motion.circle cx="200" cy="200" r="160" fill="none" stroke="url(#lineGrad)" strokeWidth="20" strokeLinecap="round" initial={{ pathLength: 0 }} animate={{ pathLength: 1, rotate: 360 }} transition={{ duration: 8, ease: "linear", repeat: Infinity }} />
      <motion.circle cx="200" cy="200" r="130" fill="none" stroke="rgba(0, 0, 0, 0.1)" strokeWidth="2" strokeDasharray="10 5" animate={{ rotate: -360 }} transition={{ duration: 60, repeat: Infinity, ease: "linear" }} />

      {/* Curved Path Animations */}
      <motion.path d={path} fill="none" stroke="url(#lineGrad)" strokeWidth="2" filter="url(#glow)" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, ease: "easeInOut", repeat: Infinity, repeatType: "reverse" }} />

      {/* Moving Rectangle with Cursor Interaction */}
      <motion.rect x={cursorPosition.x} y={cursorPosition.y} width="200" height="30" rx="15" fill="url(#lineGrad)" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.2 }} />

      {/* Central Icons and Orbiting Feature Icons */}
      ...
      <FeatureIcon icon={BarChart3} x={160} y={50} color="#8B5CF6" rotate={-360} />
      ...
      {/* Rotating Outer Circle with Dashed Stroke */}
      <motion.circle cx="200" cy="200" r="180" stroke="url(#lineGrad)" strokeWidth="2" fill="none" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 0.5 }} transition={{ duration: 2, repeat: Infinity, repeatType: "reverse" }} />
      <motion.circle cx="200" cy="200" r="190" stroke="rgba(255, 255, 255, 0.1)" strokeWidth="10" strokeDasharray="10 5" fill="none" animate={{ rotate: 360 }} transition={{ duration: 60, repeat: Infinity, ease: "linear" }} />
    </svg>
  )
}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        <motion.div className="flex flex-col lg:flex-row items-center justify-between">
          ...
          <div className="lg:w-1/2 mb-10 lg:mb-0">
            ...
            <motion.p className="text-gray-500 text-xl sm:text-2xl mb-8">
            ...
            <motion.div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
              ...
              <button onClick={handleVideoOpen} className="px-6 py-3 rounded-lg font-semibold border-2 border-white hover:border-black hover:text-black transition duration-300 inline-flex items-center justify-center">
